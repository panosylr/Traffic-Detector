...
/**
 * This java program opens  a pcap file from  and using a packet handler it goes
 * into a loop to read all the packets of the current pcap file.
 * It extracts main  attributes of the packet and  for each packet writes in an output file the
 * corresponding packet attributes.
 
 * @author Panagiotis Gialouris
 * 
 */
public class CreateMultipleStream {
    ...
    {
		// it writes in a text file a record  formed from the initial data  of a packet 
    	try
    	{
	 ...
	 static int puship(String X[], String Y[],String ips,String ipd,int numStreams)
     {
		 ...
		 // it returns the number of streams detected
           ...
           int th;
			...
			{
				if((ips.equals(X[j]) &&  ipd.equals(Y[j])) ||  (ipd.equals(X[j]) && ips.equals(Y[j])))
 		  				found = true;
			}
	 ...
	 static int  findPos(String X[], String Y[],String ips , String ipd, int numStreams ) 
	 {
        //  it finds the position of a pair (ips,ipd) or (ipd,ips) in the arrays X,Y
		//  a pair like (123.456.789.12, 100.212.10.3)  is considered identical with
	...
	static void scanTxtFile(String InFileName, String OutFilename)
	{
        ...
        {
	    int i=0;	
        FileInputStream fstream = new FileInputStream(InFileName); // txt file name  for scanning features
	    DataInputStream in = new DataInputStream(fstream);
	    BufferedReader br = new BufferedReader(new InputStreamReader(in));
   	    FileWriter results = new FileWriter(OutFilename,true); // open for append
        //Create new File. 
 	    BufferedWriter out = new BufferedWriter(results); 
        //Create new BufferedWriter out   
	    ...
	    String line;
            ...
            int caplenSent=0;
            int caplenRec=0;
            int sums[][]=new int[100][20]; // array for the totals  per stream (100 streams) 
            String SourceIp[] =new String [100]; // array with the source IPs 
            String DestinIp[] =new String [100]; // array with the destination IPs
	    ...
	    while((line=br.readLine()) != null)
		{
                    ...
                    if (istcp==1 || isudp==1 || ishttp==1)    // only for tcp/udp/http packets    
                    {  
                    	if ((IpS.equals("")==false)&& (IpD.equals("")==false))  // we ignore the packets without IPs
                    	{
                    		numOfStreams= puship(SourceIp, DestinIp,IpS,IpD, numOfStreams); //check for a new stream 
                    																		//for the pair
                    																		//SourceIp, DestinIp
		
                    	
                    	ipPos=findPos(SourceIp, DestinIp,IpS,IpD, numOfStreams);
                    	if (ipPos>0)
                    		{  // creating total per stream
                    		sums[ipPos][0]=sums[ipPos][0]+1;
                    		 if (sums[ipPos][0]==1) // the first packet of the stream
                    		 {
                    		    sums[ipPos][10]=totalSize; // minimum packet length
                    		 	sums[ipPos][11]=totalSize; // maximum packet length
                    		 }
                    		 else
                    		 {  // storing the min & max total size
                    			 if (totalSize<sums[ipPos][10])
                    				 sums[ipPos][10]=totalSize; // new minimum packet length
                    			 if (totalSize>sums[ipPos][10])
                    				 sums[ipPos][11]=totalSize; // new maximum packet length
                    		 }
                              ...
                              {
                    			  // outgoing packet
                    			  sums[ipPos][1]=sums[ipPos][1]+1; // counter of outgoing packets
                    			  sums[ipPos][5]=sums[ipPos][5]+headerLength;
                    			  sums[ipPos][14]=sums[ipPos][14]+payLoadLength;
                              ...
                              else
                              {
                            	  ...
                            	  sums[ipPos][2]=sums[ipPos][2]+1;// counter of incoming packets
                    			  sums[ipPos][6]=sums[ipPos][6]+headerLength;
                    			  sums[ipPos][15]=sums[ipPos][15]+payLoadLength;
                    	  ...
                    	  sums[ipPos][7]=sums[ipPos][7]+headerLength;// total headerbytes
                    	  sums[ipPos][16]=sums[ipPos][16]+payLoadLength; // total payload
                    	  sums[ipPos][17]=sums[ipPos][17]+totalSize; //total size
	     ...
	     { 
	    	 ...
	    	 final int  minNumPack=20; // minimum number of packet of the detected stream
	    	 if (totalPackets>=minNumPack)
	    	 	{
	    		 avPackLength=totalLength/totalPackets;
	    		 avPayload=totalPayload/totalPackets;
	    		 Record=numPackSent+","+numPackRec+","+totalPackets+","+numberAck+"," +headerBytesSent+","+headerBytesRec+ ","+totalHeaderBytes+",";
	    		 Record=Record+caplenSent+","+caplenRec+",";
	    		 Record=Record+minPackLength+","+maxPackLength+","+avPackLength+","+avPayload+","+payloadSent+","+payloadRec+","+totalPayload+","+totalLength+","+ClassName;
                 
	    		 // for each detected possible stream write a line in the arff file.
	    		 out.write(Record);
	    		 out.newLine();
	...
	public void create() 
	{
        ...
        //       System.out.println(packet.getTotalSize());//costas
         //      System.out.printf( " total size %-6d caplen= %6d  %6d  ", packet.getTotalSize(), packet.getCaptureHeader().caplen(),packet.getTotalSize()- packet.getCaptureHeader().caplen());//costas
                ...
                headerLength=udp.getHeaderLength();
           	   if (packet.hasHeader(ip))  // check for the existance of ip
           	 	 ...
           	 	 //	String sourceIP =null;  //   IP address
                     	...
                     	try 
                     	{
                    		    ipAddress =InetAddress.getByAddress(ip.source())  ;     // get the ip address  
                    	//	    ipAddress= InetAddress.getByAddress(ip.destination());
                        }catch (UnknownHostException e){}
                     	
                    	ssourceIp=ipAddress.getHostAddress(); // get the ip address
           	 		
               	 		if (sourceIp.startsWith("10.3.") || sourceIp.startsWith("192.168.")||sourceIp.startsWith("10.239.") || sourceIp.startsWith("10.4."))
           	 		{
           	 		//	System.out.print("ACK "+tcp.flags_ACK()+ " IP: "+sourceIP+ " internal");
           	 			ext=0;  // out-going message
           	 		}
           	 		else
           	 		{
           	 		//	System.out.print("ACK "+tcp.flags_ACK()+ " IP: "+sourceIP+ " external");	
...
//**********************************************                 
               ...
               {               	  
                ...
                headerLength=tcp.getHeaderLength();
            	   if (packet.hasHeader(ip))  // check for existance of ip header
               		{   
            	  //	 	String sourceIP =null;  //  
               ...
               String rec=istcp+","+ ishttp+","+isudp+","+ack+ ","+ext+","+TotalSize+ ","+PayloadLength+ ","+caplen+ ","+ headerLength+ ","+sourceIp+","+destinIp+","+user;                 
...
//		    pcap.loop(Pcap.LOOP_INFINITE, jpacketHandler, user);
		 ...
		 **************************************************************************/
		   ...
		   scanTxtFile(file,"test-final.arff"); // scans the file.txt and creates a weka file (arff)
		}
